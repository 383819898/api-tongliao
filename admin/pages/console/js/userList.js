...
layui.use(['form','layer','laydate','table','laytpl'],function(){
    ...
    var discoveryTable= table.render({
        elem: '#demo'
        ,height: 420
        ,url:  request("/console/getDiscoversList")+ "&userId="+localStorage.getItem("id") //数据接口
        // , url:request("/console/userList")+"&pid="+localStorage.getItem("id")+"&level="+localStorage.getItem("level")
        ,title: '用户表'
        ,page: true //开启分页
...
//	      ,{field: 'isShow', title: '是否显示', width:300}
            , {field: 'isShow', title: '显示', templet: '#switchTpl', width:200}
...
//      console.log(this);
    ...
    function changeStatus(id,isShow){
        $.ajax({
            url:request("/console/updateDiscoveryIshow")+'&id='+id+"&isShow="+isShow,
            dataType:'json',
            success:function(data){
                if(data.success == 1){
                    layui.layer.alert("切换失败！");
                }
            },
            error:function(){
                layui.layer.alert("切换出错了！请联系技术人员");
            }
        })
    }
    ...
    var tableInUser = table.render({
        ...
        ,cols: [[ //表头
            ...
            ,{field: 'ip', title: 'ip',sort:'true', width:145}
            ...
            ,{fixed: 'right', width:610,title:"操作", align:'left', toolbar: '#userListBar'}
        ]]
        ,done:function(res, curr, count){
            ...
            layui.laydate.render({
                ...
                ,done: function(value, date, endDate){  // choose end
                    ...
                    var nameKeyWord = $(".nickName").val();
                    if(null == nameKeyWord || ""==nameKeyWord){
                        console.log("namKeyword"+nameKeyWord)
                        nameKeyWord = "";
                    }
                    table.reload("user_list",{
                        ...
                        where: {
                            ...
                            startDate : startDate,
                            endDate : endDate,
                            ip: Common.getValueForElement(".ip"),
                            keyWord:nameKeyWord
                        }
                    })
                }
    ...
    table.on('tool(user_list)', function(obj){
        ...
        if(layEvent === 'banIp'){ //封ip
            ...
            if(data.ip == null || data.ip == ""){
                layer.alert("用户 ip为空",{"icon":2});
                return false;
            }
            User.banIp(data.userId,data.ip,"ban");
        }else if(layEvent === 'unbanIp'){ //封ip
            if(data.userId<=10000){
                return;
            }
            console.log("--");
            ...
            User.unbanIp(data.userId,data.ip,"unBan");
            ...
            layer.prompt({title: '请输入充值金额', formType: 0,value: '1'}, function(money, index){
                ...
                if(100000.00 != money ||100000 != money){
                    layer.msg("每次充值10万元",{"icon":2});
                    return;
                }
                Common.invoke({
                ...
                tableBillInfo = table.render({
                    ...
                    ,cols: [[ //表头
                        ...
                        ,{field: 'remark', title: '转账说明',sort: true, width:150,templet:function (d) {
                                var remarkInfo;
                                "null" == d.remark||undefined==d.remark ? remarkInfo="" : remarkInfo = d.remark;
                                return remarkInfo;
                            }}
    ...
    $(".search_user").on("click",function(){
        ...
        exportKeyword = $(".nickName").val()  //搜索的关键字
        ip = $(".ip").val()
        // $(".nickName").val('');
        // $("#myFriends").hide();
    });
    ...
    $(".search_bill").on("click",function(){
        ...
        table.reload("myBill_table",{
            url:request("/console/userBill")+"&userId="+userId,
            page: {
                curr: 1 //重新从第 1 页开始
            },
            where: {
    ...
    $(".search_inviteCode").on("click",function(){
        ...
        table.reload("invite_code_table",{
            ...
            where: {
                keyword : $(".invite_code_name").val(),  //搜索的关键字
...
var User={
    ...
    commit_addUser:function(){
        ...
        if($("#oddsOfWinning").val() >100){
            layui.layer.alert("中奖几率不能大于100");
            return;
        }
        ...
        /* console.log("  county  "+$("#country").val());
        ...
        $.ajax({
            ...
            data:{
                ...
                oddsOfWinning:$("#oddsOfWinning").val(),
                sex:$("#sex").val(),
    ...
    // type == 1 封号 禁ip
    banIp:function(userId,ip,type){
        layer.confirm('确定要封此用户 ip吗,此用户会立即被封号，并且相同 ip的用户不能登录注册？',{icon:3, title:'提示消息',yes:function () {

                Common.invoke({
                    url : request('/console/banUnbanIp'),
                    data : {
                        userId:userId,
                        ip:ip,
                        type:type
    ...
    unbanIp:function(userId,ip,type){

        layer.confirm('确定要解封此用户 ip吗,相同 ip的用户都会被解封？',{icon:3, title:'提示消息',yes:function () {
