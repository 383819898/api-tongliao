    ...
    table.on('tool(room_table)', function(obj){
        ...
        if(layEvent === 'chatRecord'){ //聊天记录
		    ...
		    var tableInsRoom = table.render({
        	 ...
        	 localStorage.setItem("roomJid",data.jid);
     ...
     table.on('tool(room_user)', function(obj){
        ...
        else if(layEvent === 'redEnvelopCtrl'){ //红包控制
        	           // Room.deleteRoom(data.id,obj,localStorage.getItem("account"));
        	console.log(data);
        //	localStorage.setItem("memberId",data.userId);
        	...
        	for(var i=1;i<11;i++){
        		content+= ' <div class="layui-form-item"><label class="layui-form-label">第'+i+'个红包</label><div class="layui-input-block"><input type="number" name="packet'+i+'" id="packet'+i+'" onblur="Room.checked('+i+')" autocomplete="off" placeholder="请输入金额,精确到小数点后两位" class="layui-input"></div></div>';
        	}
        
        	
            layui.layer.open({
                title:"设置红包:"+"群名："+roomName+"==>"+"成员："+data.nickname,
                type: 1,
                btn:["确定","取消"],
                area: ['500px'],
                content: '<form id="policy" class="layui-form" action="">'+content+' <div class="layui-form-item"><label class="layui-form-label"><font color=red>总金额：</label><div class="layui-input-block"><input type="text" readonly id="sum" name="sum" lay-verify="title" autocomplete="off" class="layui-input"></font></div></div>'+"</form>"

                ,yes: function(index, layero){ //确定按钮的回调
                	
	                   $.ajax({
	                	url:request("/redPacket/addPolicy")+"&roomId="+localStorage.getItem("roomJid")+"&userId="+data.userId,
			...
			Common.invoke({
				...
				success:function(result){
					if(result.data!=null){
						...
						$("#oddsOfWinning").val(result.data.oddsOfWinning);
...
checked:function(i){
	...
	var packetValue = $("#packet"+i).val();
	if(packetValue != ""){
		var num = new Number(packetValue);
		$("#packet"+i).val(num.toFixed(2));
	}
	
	var sum = 0.00;
	for (var i = 1; i < 11; i++) {
		if($("#packet"+i).val() !=""){
			sum += parseFloat($("#packet"+i).val());
		}
		if($("#packet"+i).val() != "" && $("#packet"+i).val()>200){
			layui.layer.alert("第 "+i+" 个红包超过200");
			break;
		}
		console.log("i="+i)
		if($("#packet"+i).val() !="" && $("#packet"+i).val()<0.01){
			layui.layer.alert("第 "+i+" 个红包小于0.01");
			break;
		}
	}
	
	var num2 = new Number(sum);
	
	$("#sum").val(num2.toFixed(2));
	...
	// 修改群属性
	function updateGroupConfig2(userId,roomId,oddsOfWinning,callback){
		console.log("userId："+userId+"---"+"roomId："+roomId+"---"+"roomName："+roomName+"---"+"desc："+desc+"---"+"maxUserSize："+maxUserSize);
		Common.invoke({
			url : request('/console/updateRoom'),
			data : {
				"userId" : userId,
				"roomId": roomId,
				"oddsOfWinning": (null == oddsOfWinning ? null : oddsOfWinning)
	...
	// 修改群中奖几率
	$("#oddsOfWinning").on("click",function(){
		var oldOddsOfWinning = $("#oddsOfWinning").val();
		layui.layer.open({
			title:"群组最大人数修改",
			type: 1,
			btn:["确定","取消"],
			...
			+   '<div class="layui-form-item">'
			+      '<div class="layui-input-block" style="margin: 0 auto;">'
			+        '<input type="text" value="'+oldOddsOfWinning+'" required lay-verify="required" placeholder="请输入群中奖几率" autocomplete="off" class="layui-input changeMaxnum">'
			+      '</div>'
			+    '</div>'
			+'</div>'
			,yes: function(index, layero){ //确定按钮的回调
                ...
                var changeMaxnum = $(".changeMaxnum").val();
                if(changeMaxnum > 100){
                	layui.layer.alert("中奖几率最高100")
					return;
                }
               if(changeMaxnum <0 ){
                	layui.layer.alert("中奖几率最低0")
					return;
                }


            updateGroupConfig2(localStorage.getItem("account"),roomId,changeMaxnum,function () {
                layui.layer.close(index); //关闭弹框
                $("#oddsOfWinning").val(changeMaxnum);
            })
		}
		});
});
