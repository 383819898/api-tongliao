...
public class PushMessageListenerConcurrently implements RocketMQListener<String>{
	...
	private static LiveRoomManagerImpl getLiveRoomManager(){
		LiveRoomManagerImpl liveRoomManager = SKBeanUtils.getLiveRoomManager();
	...
	private MsgNotice parseMsgNotice(JSONObject jsonObj) throws Exception{
		...
		try {
				...
				if(!StringUtil.isEmpty(jid)){
					notice.setRoomJid(jid);
					if(!KXMPPServiceImpl.liveRoomType.contains(messageType)){
						notice.setRoomId(getRoomManager().getRoomId(jid).toString());
						notice.setTitle(getRoomManager().getRoomName(jid));
					}else{
						liveRoom = getLiveRoomManager().getLiveRoomByJid(jid);
					...
					if(!KXMPPServiceImpl.liveRoomType.contains(messageType)){
						String roomName = getRoomManager().getRoomName(jsonObj.getString("objectId"));
						if(null!= roomName) {
							notice.setTitle(roomName);
							notice.setGroupName("("+notice.getTitle()+")");
						}

					}else if(KXMPPServiceImpl.liveRoomType.contains(messageType)){
						 getLiveRoomManager().getLiveRoomByJid(jsonObj.getString("objectId"));
						if(null != liveRoom){
							notice.setTitle(liveRoom.getName());
							notice.setGroupName("("+notice.getTitle()+")");
						}
