...
public class MsgRepositoryImpl extends MongoRepository<Msg,ObjectId> implements MsgRepository {
	...
	public Msg add(int userId, AddMsgParam param) {
		...
		if(null != param.getUserRemindLook()){
			if(null != param.getUserNotLook()){
				...
				logger.info("朋友圈提醒朋友列表:{}, 不给看列表:{}, 交集列表：{}",param.getUserRemindLook(),param.getUserNotLook(),JSONObject.toJSONString(collect));
		...
		for(String msgId : msgIds){
			...
			ThreadUtil.executeInThread(new Callback() {
				...
				public void execute(Object obj) {
					...
					SKBeanUtils.getRedisCRUD().delete(String.format(RedisServiceImpl.S_MSG_MSGID, msgIds));
				}
			});
		}
		return true;
	...
	**/ 
	...
	public List<Msg> getMsgList(Integer userId, ObjectId msgId, Integer pageSize,Integer pageIndex) {
		...
		Query<User> userQuery = getDatastore().createQuery(User.class).field("settings.notSeeFilterCircleUserIds").hasThisOne(userId);
		List<User> asList = userQuery.asList();
