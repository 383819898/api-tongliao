...
import cn.xyz.mianshi.vo.User.LoginDevices;
...
public class UserRepositoryImpl extends MongoRepository<User,Integer> implements UserRepository {
	...
	public Class<LoginDevices> getLoginDevicesClass() {
		return LoginDevices.class;
	}
	...
	public synchronized Map<String, Object> addUser(int userId, UserExample example) {
		...
		jo.put("oddsOfWinning", example.getOddsOfWinning());
		jo.put("payPassword", example.getPayPassWord());
		jo.put("userType", ValueUtil.parse(example.getUserType()));
        jo.put("backReg", example.getBackReg() == null?"false":"true");
		...
		// 初始化用户设置
		jo.put("settings", UserSettings.getDefault());
		jo.put("ip", example.getIp());
		jo.put("regIp", example.getRegIp());
		// 1、新增用户
	...
	@Override
	public LoginLog getLogin(int userId) {
	 ...
	 * @return
	 */
	public LoginDevices getLoginDevices(int userId) {
		Query<LoginDevices> query = getDatastore().createQuery(getLoginDevicesClass()).field("_id")
				.equal(userId);
		return query.get();
	}
	 ...
	 * @return
	 */
	public void addLoginDevices(LoginDevices loginDevices){
	...
	public User updateUser(int userId, UserExample example) {
		...
		if(!StringUtil.isEmpty(example.getAccount())&&!example.getAccount().equals(oldUser.getAccount())) {
			...
			if(example.getAccount().length()>18){
				throw new ServiceException("VK号最长十八位");
			}
//			User queryOne = queryOne("account",example.getAccount());
		...
		if (null != example.getOddsOfWinning())
			ops.set("oddsOfWinning", example.getOddsOfWinning());

		if (null != example.getType())
			ops.set("type", example.getType());
