...
//		String telephone=session.getTelephone();
	...
	**/ 
	...
	public List<AddressBook> findRegisterList(int userId,int pageIndex,int pageSize){
		...
		query.filter("telephone",user.getTelephone());
	...
	public void deleteAddressBook(Integer userId,Integer toUserId){
		Query<AddressBook> query=createQuery();
		query.field("userId").equal(userId).field("toUserId").equal(toUserId);
		if(null != query.get())
			deleteByQuery(query);
	}
	...
	private void push(Integer receiver,Integer fromUserId,String telephone,long registerTime){
		...
		json.put("fromUserName",ConstantUtil.getMsgByCode(from+"", ""));
