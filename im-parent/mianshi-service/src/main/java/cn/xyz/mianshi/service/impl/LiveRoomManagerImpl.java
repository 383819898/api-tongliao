...
public class LiveRoomManagerImpl extends MongoRepository<LiveRoom, ObjectId> {
	...
	public LiveRoom getLiveRoomByJid(String jid){
		Query<LiveRoom> query = getDatastore().createQuery(getEntityClass()).field("jid").equal(jid);
		return query.get();
	}
	...
	public void deleteLiveRoom(ObjectId roomId){
		...
		if(null !=liveRoom){
			// 删除聊天记录
			SKBeanUtils.getImRoomDatastore().getDB().getCollection(mucMsg+liveRoom.getJid()).drop();
			//删除直播间中的成员
...
//		q.filter("roomId",roomId);
		...
		User user=getUserManager().getUser(userId);
		LiveRoom liveRoom = get(liveRoomMember.getRoomId());
		if(null==liveRoomMember||liveRoomMember.getOnline()==0)
			return;
		...
		if(liveRoomMember.getType()==3&&liveRoomMember.getState()!=1){
			...
			if(liveRoom.getNumbers()<=0){
				...
				ops.inc("numbers", -1);
				updateAttributeByOps(liveRoom.getRoomId(), ops);
			}
		}else{
			if(liveRoomMember.getType()==1){
				UpdateOperations<LiveRoomMember> ops = getDatastore().createUpdateOperations(LiveRoomMember.class);
				...
				getDatastore().update(q, ops);
				Query<Black> b=getDatastore().createQuery(Black.class);
				b.filter("roomId", liveRoom.getRoomId());
				getDatastore().delete(b);
			}else{
				UpdateOperations<LiveRoomMember> ops = getDatastore().createUpdateOperations(LiveRoomMember.class);
				ops.set("online", 0);
		...
		MessageBean messageBean=new MessageBean();
		messageBean.setType(KXMPPServiceImpl.RemoveLiveRoom);
		messageBean.setObjectId(liveRoom.getJid());
		messageBean.setFromUserId(user.getUserId()+"");
		messageBean.setFromUserName(user.getNickname());
		messageBean.setToUserId(user.getUserId()+"");
...
//		User user=getUserManager().getUser(userId);
	...
	public synchronized ObjectId giveGift(Integer userId,Integer toUserId,ObjectId giftId,int count,Double price,ObjectId roomId){
		...
		if(userMoeny>=price*count){
			...
			double giftRatio = totalMoney*config.getGiftRatio();
			if(null != config)
				getUserManager().rechargeUserMoeny(toUserId,giftRatio,1);
			// 系统分成
			...
			ThreadUtil.executeInThread(new Callback() {
				...
				public void execute(Object obj) {
					...
					record.setStatus(KConstants.OrderStatus.END);
					record.setType(KConstants.ConsumeType.LIVE_RECEIVE);
					record.setPayType(KConstants.PayType.BALANCEAY); //余额支付
	...
	public void setmanage(Integer userId,int type,ObjectId roomId){
		...
		Query<LiveRoomMember> q=getDatastore().createQuery(LiveRoomMember.class);
		q.filter("userId", userId);
		LiveRoomMember livemember=q.get();
		if(null != livemember && 2 == livemember.getType())
			throw new ServiceException(KConstants.ResultCode.UserIsAdmin);
		UpdateOperations<LiveRoomMember> ops = getDatastore().createUpdateOperations(LiveRoomMember.class);
		ops.set("type", type);
		getDatastore().update(q, ops);
		//xmpp推送
