...
public class TigaseManagerImpl extends MongoRepository<Object, ObjectId>{
	...
	public void deleteUserFriendMsg(int userId){
		DBCollection collection = SKBeanUtils.getTigaseDatastore().getDB().getCollection("shiku_lastChats");
		BasicDBObject query=new BasicDBObject("userId", userId+"");
		collection.remove(query);

		DBCollection msgRepostory = getMsgRepostory();



		query=new BasicDBObject("sender", userId);
		msgRepostory.remove(query);
		DBCollection msgHistory = SKBeanUtils.getTigaseDatastore().getDB().getCollection("msg_history");
		query=new BasicDBObject("sender", userId+"@"+xmppConfig.getServerName());

		msgHistory.remove(query);
	}
