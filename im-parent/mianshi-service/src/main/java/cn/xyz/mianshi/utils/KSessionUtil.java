...
public final class KSessionUtil {
	...
	public static Map<String, Object> loginSaveAccessToken(Object userKey,Object userId,String accessToken) {
		...
		try {
			...
			int expire=KConstants.Expire.HOUR*getConfig().getAdmintokenExp();
...
//			cleanPushToken(ReqUtil.getUserId(), devicekey);
	...
	private static void removeTokenV2(String key, String access_token) {
		if (!StringUtil.isEmpty(access_token)) {
			SKBeanUtils.getRedisService().cleanUserSesson(access_token);
			log.info("  cleanUserSesson :"+access_token);
            KSession session = SKBeanUtils.getRedisService().queryUserSesson(access_token);
            SKBeanUtils.getRedisService().cleanLoginTokenKeys(session.getLoginToken());
			log.info("  cleanLoginTokenKeys :"+session.getLoginToken());
			getRedisCRUD().delete(key);
			log.info("  delete key:"+key);
			String userIdKey =String.format(GET_USERID_BYTOKEN, access_token);
			SKBeanUtils.getRedisCRUD().del(userIdKey);
			log.info("  delete GET_USERID_BYTOKEN userIdKey:"+userIdKey);
//			cleanPushToken(ReqUtil.getUserId(), devicekey);
		}
	}
