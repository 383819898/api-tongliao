layui.use(['form', 'layedit', 'laydate'], function() {
	...
	$(document).ready(function() {
		//初始加载loading框
		init_loading_index = layui.layer.load(1, {
			shade: [0.4, '#383535']
		});
	...
	$(function() {
		...
		mySdk.getUser(myData.userId, function(data) {
			loginDataObj.user = data;
			if(myFn.isNil(myData.password)) {
				myData.password = data.password;
			}
			if(myFn.isNil(myData.telephone)) {
				myData.telephone = data.telephone;
			}
	...
	function init() { //进入主页面后执行
		...
		if(myData.user.settings && myData.user.settings.openService == 1) {
		...
		$("#btnEndChat").click(function() {
			ownAlert(3, "结束会话");
			CustomerService.sendEndSession(); //给对方发送结束会话xmpp消息
		});
...
//监听收藏图片上传按钮
		...
		document.onclick = function(e) {
			...
			if(e.target.className == "xu_deimg") {
				$("#sc-panel").show();
			} else {
				$("#sc-panel").hide();
			}
		...
		$("#btnSettingOK").click(function() {
			...
			// if($("#customerService").is(':checked')==true){
			// 	myFn.switchCustomer(1);
			// }else{
			// 	myFn.switchCustomer(0);
			// }
...
//			 onclick="UI.xuscimg(\'\')  "
			success: function(result) {
				...
				for(var i = 0; i < result.data.length; i++) {
					xhtml += ' <div class="xupimg" style="position: relative;cursor: pointer;float: left;text-align: center;width:78px;margin-top: 10px;"> <img class="xu_scimg"  onclick="UI.xuupimg(\'' +  result.data[i].url + '\')"  src="' + result.data[i].url + '"  style="width: 68px;height:68px; object-fit: cover;border-radius: 2px;"/><img src="img/delete.png" class="xu_deimg" title="删除" onclick="UI.xudeimg(\'' +  result.data[i].emojiId + '\')" style="position: absolute;top: 1px;right:5px;width: 21px;height: 25;"/></div>'
				}

				$("#sc-panel").append(xhtml)
	...
	$(document).ready(function() {
		...
		$("#shaizi").tooltip({ //鼠标移动弹出提示
			trigger: 'hover',
			html: true,
			title: '骰子',
			placement: 'top'
		});
		$("#caiquan").tooltip({ //鼠标移动弹出提示
			trigger: 'hover',
			html: true,
			title: '猜拳',
			placement: 'top'
