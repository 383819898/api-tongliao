...
var UI = {
	...
	createItem: function(msg, fromUserId, direction, isSend) { //消息Item
		...
		/*if(WEBIM.isGroupChat(ConversationManager.chatType)&&0==direction){
			html=html+ "<p class='chat_nick' style='color:#8f8f8f'>" + msg.fromUserName + "</p>";
		}  

		html=html+"<div class='content'>";
		if(!WEBIM.isGroupChat(ConversationManager.chatType)) { //判断是否是群组消息
	    	 html=html +(0 == direction ? contentHtml : "<span id='msgStu_"+msg.id+"' class='msgStatus"+
	    	 	 (true == msg.isRead ? " msgStatusBG' style='background-color:#7BD286;'>已读" :1==isSend?" '><img src='img/loading.gif'>":" msgStatusBG'>送达")+ "</span>"+contentHtml);	
		}else{//群组消息
			...
			if (1==GroupManager.roomData.showRead) {//开启群消息已读  若为@消息强制显示
				...
				//readNum==0?"未读":		
				if(1==direction)																						
			 		html=html + ("<span id='groupMsgStu_"+msg.id+"' class='msgStatus msgStatusBG click' onclick='GroupManager.showReadList(\"" + msg.id + "\")'>"+(readNum+"人")+
			 			(1==isSend?"<img src='img/loading.gif'>":"")+"</span>"+contentHtml);
				else
					html=html +contentHtml+ ("<span id='groupMsgStu_"+msg.id+"' class='msgStatus msgStatusBG click' onclick='GroupManager.showReadList(\"" + msg.id + "\")'>"+(readNum+"人")+"</span>");
			}else{
			 	html=html + (0 == direction ? contentHtml : "<span id='groupMsgStu_"+msg.id+"' style='background-color:none;' > "+(1==isSend?"<img src='img/loading.gif'>":"")+" </span>"+contentHtml);
			}
	  		  //html = (1==isSend?"<img src='img/loading.gif'>":"")+html;
		} 
	...
	createMsgStatusIteam: function(msg, direction, isSend) {
		...
		if(!WEBIM.isGroupChat(ConversationManager.chatType)) {
			...
			if(1 == GroupManager.roomData.showRead) { //开启群消息已读  若为@消息强制显示
				...
				msgStatusHtml += (0 == direction ? "" : "<span id='groupMsgStu_" + msg.messageId + "'></span>" + (1 == isSend ? "" : ""));
			}
		}
	...
	uploadImg2: function() {
		...
		layui.upload.render({
			...
			url: "http://upload.quyangapp.com:8088/upload/UploadServlet",
			size: 5000,
			data: {
				id: "",
				userId: myData.userId,
			...
			choose: function(obj) {
				...
				myFn.invoke({
					url: '/user/emoji/batAddCustomize',
					data: {
						userId: myData.userId,
						imgs: res.data.images[0].oUrl,
						type: 1
					},
					async: false,
					success: function(result) {
	...
	*/
	...
	createNewFriendsItem: function(obj) {
		var _item = "<div class='media-main' style='height:60px;border-bottom:1px solid #EEF0F5'><a class='pull-left media-avatar' href='javascript:UI.showUser(" + obj.toUserId + ")' style='margin-left:10px'>" +
			...
			"</a>" +
			"<div style='cursor: pointer;' class='text-length media-body'><div style='font-size:13px;margin-top:1%'>" +
		...
		if(0 == obj.direction) {
			...
			if(500 == obj.type) {
				...
				// +"<div style='color:#7E7979;display:inline'>"+obj.content+"</div>";
			} else if(502 == obj.type) {
				_item = _item + "<div style='color:#7E7979;display:inline'>" + obj.content + "</div>";
				if(myData.userId != obj.from) {
					_item = _item + "<button onclick='UI.acceptFriends(" + obj.toUserId + ")' class='btn btn-default newFriendButton' style=''>" +
						"同意</button>";
				}
				_item = _item + "<button onclick='UI.showReplySeeHai(" + obj.toUserId + ")' class='btn btn-default newFriendButton' style=''>" +
					"回复</button>";
				...
				_item = _item + "<button class='btn btn-default newFriendButton' onclick='UI.addFriends(" + obj.toUserId + ")'  style=''>" +
					"加好友</button>";

			} else if(507 == obj.type) {
				_item = _item + "<div width=100 style='color:#7E7979;display:inline'>已拉黑</div>" +
					"<button class='btn btn-default newFriendButton' onclick='mySdk.deleteBlacklist(" + obj.toUserId + ")'  style=''>" +
					"移除黑名单</button>";
			} else {
			...
			if(500 == obj.type) {
				_item = _item + "<div style='color:#7E7979;display:inline'>" + obj.content + "</div>" +
					"<button onclick='UI.acceptFriends(" + obj.toUserId + ")' class='btn btn-default newFriendButton' style=''>" +
					"同意</button>" +
					"<button onclick='UI.showReplySeeHai(" + obj.toUserId + ")' class='btn btn-default newFriendButton' style=''>" +
					"回复</button>";
				...
				_item = _item + "<div width=100 style='display:inline;color:#7E7979'>被取消了好友 </div>" +
					"<button onclick='UI.addFriends(" + obj.toUserId + ")' class='btn btn-default newFriendButton' style=''>" +
					"加好友</button>";

			} else if(507 == obj.type) {
				_item = _item + "<div width=100 style='display:inline;color:#7E7979'>已被拉黑 </div>";
				/*+"<div  width=100 style='font-size:12px;display:inline'>";*/
				/*+"<button onclick='UI.addFriends(" + obj.toUserId + ");' class='btn btn-default' style='border:0;font-size:12px;display:inline;background-color:#55CBC4;float:right;margin-top:-15px;color:white'>"
					+"请求取消黑名单</button>";*/
	...
	showNearbyUser: function(pageIndex) {
		myFn.invoke({
			...
			data: {
				...
				account: $("#search_all_user").val() //111 
			},
	...
	showDetails: function(from, chatType, name) {
		...
		if(WEBIM.isChatType(chatType)) { //单聊
			...
			myData.isReadDel = DataMap.readDelMap[fromUserId];
