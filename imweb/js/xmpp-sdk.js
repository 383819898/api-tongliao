var SKIMSDK = {
	...
	initApi(url, userId, resource, token, pingTime, server) {
		...
		console.log("22222222222255sssss")
		console.log("userid:" + userId)
		localStorage.setItem("uid", userId); //这是用户id
		var messageId = WEBIM.createMessage(2, " ") //这是上传图片需要的id 
		localStorage.setItem("messageId", messageId.messageId)
	...
	loginIM: function(callback) {
		try {
			...
			SKIMSDK.connection = new Strophe.Connection(SKIMSDK.serverUrl, {
				'keepalive': true
			});
	...
	*/
	...
	convertToClientMsg: function(elem) {
		...
		if(ChatType.GROUPCHAT == type)
			message.chatType = WEBIM.GROUPCHAT;
		else if(ChatType.CHAT == type)
			message.chatType = WEBIM.CHAT;
		...
		if(fromResource && ChatType.CHAT == type) {
			message.from = SKIMSDK.getUserIdFromJid(message.fromUserId) + "/" + fromResource;
