...
var ApiAuthUtils = {
    ...
    /*apiAuthoLoginCreateSecret:function(obj,timeSend,url){
       // console.log("login 后的url : "+url);
        obj.userId = (myFn.isNil(obj.userId) ? myData.userId : obj.userId);
        let sortParam = EncryptUtils.paramKeySort(obj);
        //console.log("login 后参数排序 ："+sortParam);
        if(myFn.isNil(timeSend))
            timeSend = getCurrentMilliSeconds();
        obj.secret = AppConfig.apiKey+myData.userId+myData.access_token+sortParam+timeSend;
       // console.log("login 后secret ："+obj.secret);
        //console.log("login 后 httpKey ："+myData.httpKey);
    ...
    apiAuthoLoginCreateSecret(obj,timeSend,url,userId,access_token,httpKey){
        ...
        if(myFn.isNil(timeSend))
            timeSend = getCurrentMilliSeconds();
        obj.secret = AppConfig.apiKey+userId+access_token+sortParam+timeSend;
        console.log("login 后 httpKey ："+httpKey);
