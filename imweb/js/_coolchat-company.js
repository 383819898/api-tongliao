			...
			for (var i = 0; i < length; i++) {
				...
				if (department.parentId == rootDepartId) {
					...
					var departHtml = "<div id='department_"+department.id+"' style='position: relative;'> "
							        ...
							        +   "<ul id='departMenu_"+department.id+"' class='departMenu'  onmouseout='CompanyManager.hideDepartmentMenu(\"" + department.id + "\",event);'>" 

							        if (employeeRole[department.companyId]>=1) //权限判断
							        	departHtml += "<li style='' onclick='CompanyManager.showCreateChildDepartment(\"" +department.companyId+ "\",\"" + department.id + "\")'>创建子部门</li>"
												   + "<li style=''  onclick='CompanyManager.delEmployee(\"" +department.companyId+ "\",\"" + department.id + "\")'>删除员工</li>"
							        			   + "<li style=''  onclick='CompanyManager.quitDepartment(\""+department.id+"\",\""+department.companyId+"\")'>删除部门</li>";

							        departHtml += "<li  onclick='CompanyManager.addEmployee(\"" + department.id + "\",\"" + department.companyId + "\")'>添加员工</a>"
							        +   "</ul>"
				...
				success : function(result) {
					if (1 == result.resultCode) {
						...
						var departHtml = "<div id='department_"+department.id+"'> "
							        ...
							        +   "</div>"  
							        +   "<div id='departMenu_"+department.id+"' class='departMenu' style='opacity: 0.5;position: absolute; right: 1%; display: none;' onmouseout='CompanyManager.hideDepartmentMenu(\"" + department.id + "\",event);'>" 
							       
							        if (employeeRole[department.companyId]>=1) //权限判断
							        	departHtml += "<a href='#' onclick='CompanyManager.showCreateChildDepartment(\"" +department.companyId+ "\",\"" + department.id + "\")'>创建子部门</a><br/>"
							        			   +"<a href='#' onclick='CompanyManager.delEmployee(\"" +department.companyId+ "\",\"" + department.id + "\")'>删除员工</a><br/>"
							        			   +"<a href='#' onclick='CompanyManager.quitDepartment(\""+department.id+"\",\""+department.companyId+"\")'>删除部门</a><br/>";
				...
				success : function(result) {
					if (1 == result.resultCode) {
						...
						var childDepartHtml = "<div id='department_"+department.id+"'> "
							        ...
							        if (employeeRole[department.companyId]>=1) //权限判断
							        	childDepartHtml += "<a href='#' onclick='CompanyManager.showCreateChildDepartment(\"" +department.companyId+ "\",\"" + department.id + "\")'>创建子部门</a><br/>"
							        			   +"<a href='#' onclick='CompanyManager.delEmployee(\"" +department.companyId+ "\",\"" + department.id + "\")'>删除员工</a><br/>"
							        			   +"<a href='#' onclick='CompanyManager.quitDepartment(\""+department.id+"\",\""+department.companyId+"\")'>删除部门</a><br/>";
