...
var WEBIM={
    ...
    handlerMessageByType:function(message){
    	...
    	if(-1 == message.objectId){
            ownAlert(3, "该账号已被后台管理员锁定封号");
            setMyData.updateOrResetPassword();
            return;
        }
		...
		switch (message.type){
		 case MessageType.Type.CONSOLELOCKUSER:// 后台管理锁定用户封号处理
             WEBIM.handlerConsoleLockUserMessage(message);
             break;
	...
	decryptMessage:function(msg) {
		...
		if(WEBIM.isEncrypt(msg)&&msg.content){
    ...
    initConfig:function(){
		mySdk.getConfig(function(result){
			...
			*/
			...
			if(!myFn.isNil(result.XMPPDomain)){
				AppConfig.boshDomain=result.XMPPDomain;
				AppConfig.mucJID="muc."+AppConfig.boshDomain;
			}
