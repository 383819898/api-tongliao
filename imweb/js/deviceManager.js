...
var DeviceManager = {
	...
	initMyDevices:function(){
	    ...
	    var my_device_html ='<div class="" id="friends_MyDevice" onclick="DeviceManager.showMyDevice();">'
					       ...
					       +     	'<div class="ext"> <p id="timeSend_10002" class="attr"></p> </div>'
						   +        '<div class="avatar">'
	...
	createDevicesItem : function(imgUrl, userId, nickname,content,key,time) {
		/*var _item = "<div class='' id='friends_"+userId+"_"+key+"' onclick='UI.isChoose(\"" + userId+"_"+key + "\");'>" 
				    ...
				    +      "</a>"
				    +      "<div onclick='ConversationManager.open(\"" + (userId + "@" + AppConfig.boshDomain+"/"+key) + "\",\"" + nickname + "\");' style='cursor: pointer;' class='media-body'>"
				    +         "<h5 class='media-heading'>" + nickname + "</h5>"
				   	+(myFn.isNil(time) ? " " :" <span class='timeSend_"+userId +" timeSend'>" + time + "</span>")
				    +" <div id='msgNum_"+userId+"_"+key+"' class='news' style='float:right;display:none'><span id='span' style='background-color: #FA6A43;border-radius: 12px;padding:0 4px;color:white;'>"
				    +1+"</span></div>"
				    +         "<div class='msgListTitle' id='titfriends_"+userId+"_"+key+"' style='color:#7E7979'>" +content+ "</div>"
				    +      "</div>"
				    +   "</div>"
				    +"</div>";*/
		...
		var _item = "<div  class='' id='friends_"+userId+"_"+key+"' onclick='UI.isChoose(\"" + userId+"_"+key + "\");'>"
				  +     "<div class='chat_item slide-left active'>"
				  +          "<div class='ext'> <p class='attr timeSend_"+userId +"' >" + time + "</p> </div>"
				  +          "<div class='avatar'>"
