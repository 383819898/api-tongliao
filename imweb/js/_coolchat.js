...
var ConversationManager = {
	...
	showAvatar : function(userId,isGroup,deviceKey){ //显示聊天窗口顶部头像和昵称
		...
		if(isGroup==0){//单聊
			...
			avatarHtml="<div class='imgAvatar'>"
			           +	"<figure style='height:40px;width:40px;'>"
		               +	  "<img src='./img/group_avatar.png' class='chat_content_avatar' onclick='msgMabayRoom()'>"
		               +	"</figure>"
		               +"</div>";
		    $("#chatAvator").empty();
		    $("#desphoto").empty();
	...
	showHistory : function(pageIndex, cb,endTime) {
		...
		var url = WEBIM.GROUPCHAT!= ConversationManager.chatType ? '/tigase/shiku_msgs' : '/tigase/shiku_muc_msgs';
		var params = {
			pageIndex : pageIndex,
			pageSize : 20,
			endTime : parseInt(endTime*1000),
			maxType: WEBIM.GROUPCHAT == ConversationManager.chatType?0:200,
		};
		params[WEBIM.GROUPCHAT!= ConversationManager.chatType? "receiver" : "roomId"] = WEBIM.getUserIdFromJid(ConversationManager.from);
		myFn.invoke({
			url : url,
			data : params,
			success : function(result) {
	...
	processMsg:function(message){
		...
		if(1==myData.multipleDevices){
			//// 好友消息处理
			if(WEBIM.CHAT == msg.chatType&&fromUserId!=myData.userId){
				DeviceManager.receiverMessage(msg);
			}else if(WEBIM.CHAT == msg.chatType&&fromUserId==myData.userId){
				//其他 设备消息处理
		...
		if(WEBIM.CHAT==msg.chatType&&myData.userId==msg.fromUserId){
	...
	handlerAudioOrVideoMessage:function(msg){
		...
		if(WEBIM.isGroupChat(msg.chatType)&&myData.userId==msg.fromUserId){
			...
			return true;
		}else if(WEBIM.isChatType(msg.chatType)&&1==myData.multipleDevices && myData.userId!=WEBIM.getUserIdFromJid(msg.from)){
				DeviceManager.receiverMessage(msg);
		}
		Call.processMsg(msg);
	},
	...
	sendReadReceipt : function(from,messageId,isSendMyDevice) {
		...
		msg.reSendCount=3;
		DataUtils.saveMessage(msg);
		shikuLog("发送已读回执："+msgObj.messageId+"       类型:"+msgObj.type);
	...
	checkEncrypt:function(msg,callback){
		//检测消息加密  如果加密 调用接口加密
		var content=msg.content;
		if(msg.type!=MessageType.Type.FEEDBACK && myData.isEncrypt==1){
