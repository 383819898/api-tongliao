	...
	//$(function() {
		...
		form.on('submit(user_login)', function(obj) {
	       ...
	       /* obj.field.areaCode = $("#telephone").intlTelInput("getSelectedCountryData").dialCode;
	        ...
	        if(myFn.isNil(obj.field.areaCode)){
	        	obj.field.areaCode = "86";  //兼容用户名注册没有区号
	        }
...
function checkTelephone(telephone){
	...
	var type = localStorage.getItem("regeditPhoneOrName");
	if(type == 1){
		return true;
...
console.log("show_area_id = "+show_area_id+" tel_input_id="+tel_input_id)
	var areaCode = $("#"+tel_input_id+"").intlTelInput("getSelectedCountryData").dialCode;

    if(myFn.isNil(areaCode)){
    	areaCode= "86";
    }
	var telephone = $("#"+tel_input_id).val();
	
	if(localStorage.getItem("regeditPhoneOrName") == 1){
		alert(AppConfig.apiUrl+"/getImgCode?telephone="+areaCode+telephone+"&n="+Math.random())
