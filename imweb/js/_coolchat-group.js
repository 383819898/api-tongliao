	...
	myFn.invoke({
		...
		success: function(result) {
			...
			for(var i = 0; i < fileList.length; i++) {
				...
				tbInviteListHtml += "<tr><td width=100%>" + obj.name + "</td>";
				if(myData.userId == obj.userId || 3 > GroupManager.roomData.member.role)
					tbInviteListHtml += "<td><button class='btn btn-default' onclick='GroupManager.deletefile(\"" + obj.shareId + "\")'>删除</button></td>";
				tbInviteListHtml += "<td><a target='_blank' href='" + obj.url + "'download='" + fileName + "' filename='" + fileName + "' alt='" + fileName + "'" +
					"style='color:white'><button class='btn btn-default'>查看</button></a></td>" +
					"</tr>";
			}
			$("#gfile").empty();
...
var GroupManager = {
	...
	//上传
	uploadFile: function() {
		$("#myFilePreviewPrant #myFilePreview").empty();
		...
		switch(msg.contentType) {
			...
			case 902:
				if(ConversationManager.isOpen && msg.roomJid == ConversationManager.fromUserId) {
					$("#myRoomList #groups_" + msg.objectId + " .nickname_text").html(msg.text);
					$("#myMessagesList #groups_" + msg.objectId + " .nickname_text").html(msg.text);
					$("#gname").html(msg.text);
			...
			case 907:
				...
				if(WEBIM.isUserId(msg.toUserId) && !WEBIM.isGroupType(msg.chatType)) {
					mySdk.getRoomOnly(msg.fileName, function(obj) {
						DataMap.myRooms[obj.jid] = obj;
						WEBIM.joinGroupChat(msg.objectId, myData.userId, 0);
	...
	createGroupChat: function(groupId, userId, jid) { //groupId :群组id    userId：创建者的userId
		...
		//111
		localStorage.setItem("jid" , jid)	 //111 收藏表情用的
		$("#shaizi").show()
		$("#caiquan").show();
		//11
		UI.clearMsgNum(jid);
	...
	createGroup: function() {
		...
		SKIMSDK._XEP_0045_143(groupId, groupName, groupDesc, myData.userId, function(status, reason) {
			if(0 != status)
				return;
			mySdk.createRoom(params, function(result) {
				...
				WEBIM.joinGroupChat(groupId, myData.userId);
	...
	joinRoom: function(roomId) {
		...
		mySdk.joinRoom(roomId, function() {
			ownAlert(3, "加入群成功！");
			//加入群聊
			WEBIM.joinGroupChat(room.jid, myData.userId);
		...
		if(num == 0) {
			...
			for(let i = 0; i < readList.length; i++) {
				...
				let userHtml = '<table id="" onclick="" style="border-radius:6px;">' +
					...
					+
					'<img  alt="" src="' + myFn.getAvatarUrl(user.userId) + '" class="roundAvatar" width="40" height="40">' +
					'</a>' +
					'</td>' +
