...
var MpChat = {
	...
	**/
	loadHistoryMsg : function(to,type){
		...
		if(type==1 && (localMsgRecordList.size()>0 || !mpCommon.isNil(localUnReadMsgList) )){
			...
			//加载历史消息
			this.showHistory(0, function(status, result) {
				if (0!= status) {
					ownAlert(2,result);
				}
	...
	showLoadHistoryIcon : function(type){ // type:1 查看更多消息   type:2 loading  type:3 没有更多消息了
		...
		if (1==type) { //查看更多消息
			logHtml += "<img src='images/msgHistory.png' style='width:25px; height=25px;display:inline;'>"
					+  "<a href='#' style='font-size: 12px;' onclick='MpChat.loadHistoryMsg("+MpChat.toUserId+",2);'>查看更多消息</a>";
	...
	showHistory : function(pageIndex, cb,endTime) {
		...
		var url = mpCommon.apiAddr+'/tigase/shiku_msgs';
		var params = {
			pageIndex : pageIndex,
			pageSize : 20,
	...
	createItem : function(msg, fromUserId, direction,isSend) {   //消息Item
		...
		var timeSendStr = "";
		//10分钟
		if(delayTime>(10*60)||delayTime< -(10*60)){
	...
	createMsgContent:function(msg,direction,isSend){
			...
			switch (msg.type){
				...
				case 202://消息撤回
					...
					msgText = (msg.fromUserId==getLoginData().userId) ? "你撤回了一条消息" : msg.fromUserName+"撤回了一条消息";               
					
					recallHtml = "<div class='logContent'><span>"+msgText+" ("+getTimeText(msg.timeSend,0,1)+") </span></div>";
	...
	createMsgStatusIteam:function(msg,direction,isSend){
		...
		msgStatusHtml += ( 0 == direction ? "" : 1==isSend ? "<span id='msgStu_"+msg.messageId+"'  class='msg_status_common send_status'></span>"
			...
			(true == msg.isRead ?"<span class='msg_status_common send_status read_bg'>已读</span>" :
				(1 == msg.isReadDel ? "<img class='msg_status_readDel ico_readDel' src='./images/fire.png'><span  id='msgStu_"+msg.messageId+"' class='msg_status_readDel send_status ok_bg'>送达</span>" :
		...
		//检查当前页面是否存在发送者的记录，如有，更新最后一条消息及未读数量到页面
	   	if($("#msg_manager").is(':visible')){//判断消息管理页面是否显示  显示：true 隐藏：false
	   		...
	   		if(!mpCommon.isNil(itemHtml)){//在当前页面存在,则更新时间、未读消息数、最后一条消息等数据到页面
	   			...
	   			$("#msgListItem_"+userId+" #message_time").text(getTimeText(msg.timeSend,0,1));
	   			...
	   			var messageItemHtml = UI.createMessageItem(userId,(msg.fromUserId==getLoginData().userId) ? msg.toUserName : msg.fromUserName,1,getTimeText(msg.timeSend,0,1), WEBIM.parseShowMsgTitle(msg));
	...
	**/
	...
	handlerLoginConflict:function(){
		WEBIM.disconnect();
		ownAlert(4,"您的账户在其他地方登陆,已被挤下线！",function(){
			setTimeout(function() {
