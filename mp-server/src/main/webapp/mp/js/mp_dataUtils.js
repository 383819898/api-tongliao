...
var mpDataUtils = {
	...
	*/
	...
	getMsgNumCount:function(){
		var num=dbStorage.getItem("msgUnReadNumCount");
	...
	setMsgMumCount:function(num){
		...
		dbStorage.setItem("msgUnReadNumCount",num);
	},
	//获取某个用户的消息未读数量
	getMsgNum:function(userId){
		
		var msgNumMap = dbStorage.getItem("msgUnReadNum");
	...
	setMsgNum:function(userId,num){
		...
		dbStorage.setItem("msgUnReadNum",JSON.stringify(msgNumMap));
	},
	...
	* 消息列表的顺序
	*/
	putMsgListOrder:function(userId){
		var msgListOrder = this.getMsgListOrder();
		msgListOrder.remove(userId);
		msgListOrder.unshift(userId);
		
		dbStorage.setItem("msgListOrder",JSON.stringify(msgListOrder));
	},
	getMsgListOrder:function(){
		var msgListOrderStr = dbStorage.getItem("msgListOrder");
