	...
	$(function() {
		...
		$("#mapSubmit").click(function(){
			if(mpCommon.isNil(BaiduMap.lng)||mpCommon.isNil(BaiduMap.lat)){
				ownAlert(3,"请先选择位置!");
				return;
			}
	...
	function mapInit(){
			...
			map = new BMap.Map("baiduMap_showArea",{minZoom:4,maxZoom:18,enableMapClick:false});
			BaiduMap.map=map;
	...
	function removeMarker(){
		if(mpCommon.notNull(BaiduMap.marker))
			map.removeOverlay(BaiduMap.marker);
	}
	function getAddress(pt,cb){
		if(mpCommon.isNil(BaiduMap.myGeo)){
			BaiduMap.myGeo = new BMap.Geocoder();
		}
	...
	function showToMap(obj){
		...
		layer.open({
            ...
            area:  ['auto', 'auto'],
            content: $("#baiduMap"),//自定义 html 内容，注意这里的整个 html 内容背景肯定是透明的
